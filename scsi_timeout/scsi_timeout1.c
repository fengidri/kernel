...
static int scsi_timeout_set_by_name(int timeout, const char *buf)
{
    ...
    if (!bdev)
    {
        printk(KERN_INFO "not found block device: %s\n", buf);
        return -1;
    }

   blk_queue_rq_timeout(bdev->bd_queue, timeout * HZ / 1000);
   bdput(bdev);
...
static int scsi_timeout_proc_show(struct seq_file *m, void *v)
{
    ...
    while ((dev = bus_find_device(&scsi_bus_type, pre_dev, NULL, always_match)))
    {
        ...
        sdev = to_scsi_device(dev);
        disk = dev_to_disk(&sdev->sdev_dev);

        seq_printf(m, "%s driver: %s type: %s id:%d timeout: %dms %s \n",
                dev_name(dev),
                dev->driver->name,
                dev->type->name,
                sdev->id,
